...
import { Check, X, RotateCcw, History, Save, Info, Radio, Grid2x2, Phone, List, ArrowUpCircle, ListOrdered, Settings2, Headset} from "lucide-react";
import { Button } from "@/components/ui/button";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle} from "@/components/ui/alert-dialog";
import { AppShell } from "@/components/layout/AppShell";
import { cn } from "@/lib/utils";
import { TabBtn, UNIQUE_CATEGORIES, MAX_PORTARIA_ATENDEDOR, isBlockedSource, isBlockedHotlineTarget, isBlockedDesvioTarget, makeDefault, type Category, type Ramal, type TabId, type RamaisCount,
} from "@/components/ramais/shared";
import { RamaisPage } from "@/pages/AbasProgramaçõesDoPABX/RamaisPage";
import { RamaisEspeciaisPage, defaultPortariaHorarioState, type PortariaHorarioState} from "@/pages/AbasProgramaçõesDoPABX/RamaisEspeciaisPage";
import { InterfonesPage, defaultInterfonesState, isInterfonesDefault, type InterfonesState} from "@/pages/AbasProgramaçõesDoPABX/InterfonesPage";
import { GeraisPage, defaultGeraisState, isGeraisDefault, type GeraisState,} from "@/pages/AbasProgramaçõesDoPABX/GeraisPage";
...
export function ProgramacoesPage() {
  ...
  const [activeWindow, setActiveWindow] = useState<"programacoes" | "planoNumeracao">("programacoes");
  ...
  const showToast = (msg: string, variant: "info" | "error" = "info") =>
    setToast({ msg, variant });

  // Restaura apenas a aba atual
  const doRestore = () => {
    if (tab === "gerais") {
  ...
  // Identifica se a aba atual já está nos valores default (para desabilitar Restaurar)
  ...
  const isCurrentTabDefault = (() => {
    ...
    if (tab === "ramais" || tab === "ramais_especiais") {
      const def = JSON.stringify(makeDefault(gerais.qtd));
      return JSON.stringify(rows) === def;
    }
    if (tab === "interfones") return isInterfonesDefault(interfones);
    return true;
  })();
  ...
  const handleRestoreClick = () => {
    if (restoreDisabled) {
      showToast("Não há alterações para restaurar nesta aba.");
      return;
    }
    setRestoreOpen(true);
  };
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (["button", "submit", "reset", "image", "checkbox", "radio", "file", "color", "range", "hidden"].includes(target.type)) return;
      if (target.closest("[aria-expanded='true'], [role='combobox'], button, a, textarea, select, [role='button'], [role='link']")) return;
  ...
  }, [restoreOpen, hasChanges, handleSave]);

  // Atualiza a categoria de um ramal e limpa os campos relacionados caso a nova categoria seja um tipo bloqueado.
  ...
  setRows((prev) => {
    ...
    return prev.map((r) => {
      if (r.numeroFixo === numeroFixo) {
        const next: Ramal = { ...r, categoria: cat };
      ...
      if (
        ...
        cleaned = { ...cleaned, hotline: "nenhum" };
      }
      ...
      <div className="flex-1 min-h-0 flex flex-col">
       <div className="flex-1 min-h-0 px-10 py-6 overflow-y-scroll overflow-x-hidden overscroll-contain">
        ...
        <div className="shrink-0 border-t bg-card px-6 py-3 flex items-center justify-end gap-3 shadow-[0_-2px_10px_rgba(0,0,0,0.04)]">
          <Button
            variant="outline"
            className={cn("gap-2", restoreDisabled && "opacity-50 cursor-not-allowed")}
          ...
          <Button variant="outline" className="gap-2">
            <History className="w-4 h-4" /> Alterações
          </Button>
          <Button variant="outline" className="gap-2" disabled>
            <Save className="w-4 h-4" /> Salvar
          </Button>
          <Button variant="outline" className="gap-2" disabled>
            <ArrowUpCircle className="w-4 h-4" /> Enviar programações
          </Button>
        </div>
